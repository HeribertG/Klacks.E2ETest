...
using static Klacks.E2ETest.Constants.SettingsOpenrouteIds;
...
namespace Klacks.E2ETest
{
    ...
    public class SettingsOpenrouteTest : PlaywrightSetup
    {
        ...
        private const string TestApiKey = "test-openroute-key-e2e";
        ...
        [Test]
        [Order(1)]
        public async Task Step1_VerifyOpenrouteFormLoaded()
        {
            TestContext.Out.WriteLine("=== Step 1: Verify Openroute Form Loaded ===");

            var form = await Actions.FindElementById(Form);
            Assert.That(form, Is.Not.Null, "Openroute form should be visible");

            var header = await Actions.FindElementById(Header);
            Assert.That(header, Is.Not.Null, "Openroute header should be visible");

            var apiKeyInput = await Actions.FindElementById(ApiKey);
            Assert.That(apiKeyInput, Is.Not.Null, "Openroute API key input should be visible");

            var toggleBtn = await Actions.FindElementById(ApiKeyToggle);
            Assert.That(toggleBtn, Is.Not.Null, "Openroute API key toggle should be visible");

            Assert.That(_listener.HasApiErrors(), Is.False,
                $"No API errors should occur. Error: {_listener.GetLastErrorMessage()}");

            TestContext.Out.WriteLine("Openroute form loaded successfully");
        }
